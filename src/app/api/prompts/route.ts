...
const MEDIA_POOL: Pick<
  PromptItem,
  'src' | 'poster' | 'width' | 'height' | 'aspectRatio' | 'mediaType'
>[] = [
  {
...
const ALL_ITEMS: PromptItem[] = Array.from({ length: TOTAL_ITEMS }, (_, i) =>
  generateItem(i),
);
...
function sortItems(items: PromptItem[], sort: SortKey): PromptItem[] {
  ...
  switch (sort) {
    ...
    default:
      return sorted.sort(
        (a, b) =>
          b.views * 0.4 + b.likes * 0.6 - (a.views * 0.4 + a.likes * 0.6),
      );
  }
}
